...
import ChangeDateTable from "./changeDateTable";
 ...
 async componentDidMount() {
  ...
  getDatesList = async () => {
    ...
    moveToAdjPage=(pageName)=>{
      ...
      this.props.push({
        pathname:"/progList/"+pageName,
        state: { program_id:getItem('progName').id,upload_id:this.state.uploadID}
      })
    }
    render() {
                ...
                <div className="col-md-12">
                  <div className="kt-section">
                    <div className="kt-section__content text-center">
                      <ChangeDateTable tableData={this.state.list}/>
                    </div>
                  </div>
                </div>
              </div>
